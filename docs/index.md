...
Jayess is a JavaScript-like native programming language compiled with Go and an LLVM-based backend.

This docs folder covers the language and runtime surface currently implemented in this repository.
...
- Jayess aims for JavaScript-like syntax, but it is not the JavaScript runtime.
- The compiler produces native executables.
- `var` and `const` are supported. `let` is not supported.
